...
const ThemeToggle: React.FC<ThemeToggleProps> = ({ location, className }) => {
  ...
  useEffect(() => {
    ...
    const handleSystemChange = () => {
      ...
      const currentThemePreference = localStorage.getItem('theme') as ThemePreference || 'system';
      ...
      <div className="relative" ref={dropdownRef}>
        ...
        {isDropdownOpen && (
          <div
            ...
            className="absolute right-0 mt-2 w-36 rounded-md shadow-lg bg-white dark:bg-secondarybox ring-1 ring-black ring-opacity-5 focus:outline-none z-10" // Added z-index
            role="menu"
          ...
          >
            <div className="py-1" role="none">
              <button
                onClick={() => handleThemeChange('system')}
                className={`flex items-center gap-2 px-4 py-2 text-sm text-neutral-700 dark:text-neutral-300 hover:bg-lightgray dark:hover:bg-primarybox w-full text-left ${theme === 'system' ? 'bg-lightgray dark:bg-primarybox' : ''}`} role="menuitem" tabIndex={-1}>
                <IoContrastOutline className="size-4" />
                System
              </button>
              <button
                onClick={() => handleThemeChange('light')}
                className={`flex items-center gap-2 px-4 py-2 text-sm text-neutral-700 dark:text-neutral-300 hover:bg-lightgray dark:hover:bg-primarybox w-full text-left ${theme === 'light' ? 'bg-lightgray dark:bg-primarybox' : ''}`} role="menuitem" tabIndex={-1}>
                <IoSunnyOutline className="size-4" />
                Light
              </button>
              <button
                onClick={() => handleThemeChange('dark')}
                className={`flex items-center gap-2 px-4 py-2 text-sm text-neutral-700 dark:text-neutral-300 hover:bg-lightgray dark:hover:bg-primarybox w-full text-left ${theme === 'dark' ? 'bg-lightgray dark:bg-primarybox' : ''}`} role="menuitem" tabIndex={-1}>
