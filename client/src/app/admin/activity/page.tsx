...
// Import Types
import activityAdminService, { ActivityItem, RecentActivityApiResponse } from '../../services/admin/activity.admin'; // Adjust path
...
export default function AllRecentActivityPage() {
    ...
    const fetchAllActivitiesData = useCallback(async (isRefreshAction = false) => {
        if (!token) {
            setError("Authentication required.");
            setLoading(false);
            setIsRefreshing(false);
        ...
        try {
            ...
            const response = await activityAdminService.getRecentActivities(100000, 1);
    ...
    const { currentActivitiesToDisplay, calculatedTotalPages } = useMemo(() => {
        ...
        const paginatedData = sortedActivities.slice(indexOfFirstActivity, indexOfLastActivity);
        ...
        return { currentActivitiesToDisplay: paginatedData, calculatedTotalPages: pages > 0 ? pages : 1 };
    ...
    const paginate = (pageNumber: number) => {
         ...
         else if (pageNumber < 1 && totalPages > 0) setCurrentPage(1); // Go to first page if out of lower bounds
         else if (pageNumber > totalPages && totalPages > 0) setCurrentPage(totalPages); // Go to last page if out of upper bounds
    };
    const goToPreviousPage = () => paginate(currentPage - 1);
    const goToNextPage = () => paginate(currentPage + 1);
    ...
    const activityTypeOptions = useMemo(() => {
        return ['all', 'NEW_USER', 'NEW_PAYMENT', 'NEW_TRANSFER', 'KYC_PENDING', 'KYC_VERIFIED', 'KYC_REJECTED', 'SUPPORT_MESSAGE'];
    }, []);
    ...
    const endIndex = Math.min(currentPage * activitiesPerPage, sortedActivities.length);
                ...
                <div className="space-y-6">
                    ...
                    <div className="flex flex-wrap justify-between items-center gap-4">
                        ...
                        <div className="flex items-center gap-3 justify-end sm:w-auto w-full">
                             ...
                             <button onClick={refreshData} disabled={isRefreshing || loading} className="flex items-center justify-center cursor-pointer gap-2 bg-lightgray hover:bg-lightborder dark:bg-primarybox dark:hover:bg-secondarybox text-neutral-900 dark:text-white px-8 py-3 h-12.5 sm:w-auto w-full rounded-full transition-all duration-75 ease-linear disabled:opacity-50 disabled:cursor-not-allowed" title="Refresh activity feed">
                                 <RefreshCw className={`size-5 ${isRefreshing || loading ? "animate-spin" : ""}`} />
                                 <span>Refresh</span>
                             </button>
                         </div>
                    </div>
                    ...
                    <div className="flex flex-wrap justify-between items-center gap-4 mb-4">
                        ...
                        {!loading && ( // Only show if not initial loading
                            <p className="text-sm text-gray-500 dark:text-gray-300">
                                Showing {startIndex} - {endIndex} of {sortedActivities.length} results
                                {totalPages > 1 && ` (Page ${currentPage} of ${totalPages})`}
                            </p>
                        )}
                    </div>
                    ...
                    {totalPages > 1 && !loading && currentActivitiesToDisplay.length > 0 && (
                        <Pagination
                            currentPage={currentPage}
                            totalPages={totalPages}
                            paginate={paginate}
