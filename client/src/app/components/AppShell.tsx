...
export default function AppShell({ children }: AppShellProps) {
    ...
    useEffect(() => {
        // ... (your existing useEffect logic is fine)
        if (originalBodyOverflowRef.current === null) {
            originalBodyOverflowRef.current = document.body.style.overflow || '';
        }
        if (showAppPreloader) {
    ...
    return (
        <>
            ...
            {/* 
              FIX: Always render AuthProvider and its children.
              The AppPreloader is a sibling element with a high z-index, so it will
              act as an overlay. This prevents unmounting the page content and
