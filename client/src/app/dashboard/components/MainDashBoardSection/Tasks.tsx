...
const TasksPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchPendingPayments = async () => {
      ...
      try {
        ...
        // Check if the component is still mounted before setting the error
        if (isMounted.current) {
          setError(errMsg);
        }
        console.error("TasksPage: Error fetching pending payments:", err);
      } finally {
        // Check if the component is still mounted before setting loading state
        if (isMounted.current) {
          setIsLoading(false);
        }
        console.log("TasksPage: Fetch finished.");
  ...
  // --- Effect to Update allTasksVisible ---
  useEffect(() => {
    // Only update allTasksVisible if we actually have tasks
    if (pendingPayments.length > 0) {
      setAllTasksVisible(visibleTaskCount >= pendingPayments.length);
    } else {
      setAllTasksVisible(false); // No tasks, so not all visible
  ...
  // --- Early Return Condition ---
  // This prevents rendering anything at all (null) if there are no pending payments.
  // It is NOT the source of the #185 error, but good practice
  if (!isLoading && !error && pendingPayments.length === 0) {
    console.log("TasksPage: No pending tasks found, rendering null.");
    return null; // Render absolutely nothing
  }
