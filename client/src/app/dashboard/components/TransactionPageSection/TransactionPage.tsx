...
const TransactionsPage: React.FC = () => {
  ...
  // State to hold the currently applied filters
  const [appliedRecipientFilters, setAppliedRecipientFilters] = useState<
    (string | number)[]
  >([]);
  ...
  const [appliedFromDateFilter, setAppliedFromDateFilter] = useState<
    string | undefined
  >(undefined);
  const [appliedToDateFilter, setAppliedToDateFilter] = useState<
    string | undefined
  >(undefined);
  // Optional: Add state for search query if search needs to be combined with filters more tightly
  // const [searchQuery, setSearchQuery] = useState<string>('');
  ...
  const handleFiltersApply = useCallback(
    (filters: {
      ...
      if (statusFilter) {
        ...
        tempFiltered = tempFiltered.filter((tx) => {
          ...
          // Map UI filter names to backend statuses (ensure these match your backend)
          if (lowerCaseStatusFilter === "completed")
            return txStatus === "completed";
          if (lowerCaseStatusFilter === "cancelled")
            return txStatus === "canceled" || txStatus === "cancelled"; // Allow both spellings
          if (lowerCaseStatusFilter === "in process")
        ...
        tempFiltered = tempFiltered.filter((tx) => {
          ...
          const recipientId =
            typeof tx.recipient === "object" && tx.recipient?._id
              ...
              ? tx.recipient
              : null; // Handle recipient being just an ID string
      ...
      if (fromDateObj || toDateObj) {
        tempFiltered = tempFiltered.filter((tx) => {
          ...
          try {
            ...
            if (isNaN(transactionDateObj.getTime())) {
              console.warn(
                "Invalid transaction date string:",
                transactionDateStr
              );
              return false; // Exclude if date is invalid
            }
            ...
            console.error(
              "Error parsing transaction date for filtering:",
              transactionDateStr,
              e
            );
            return false; // Exclude on parsing error
    ...
    [allTransactions]
  ); // Dependency: Re-run filter logic if the base list changes

  // --- Transaction Grouping Logic (Optimized with useMemo) ---
  const { inProgressTransactions, groupedProcessedTransactions } =
    useMemo(() => {
      const inProgress = filteredTransactions.filter(
        (tx) => tx.status === "in progress" || tx.status === "pending"
      );
      ...
      const grouped = sortedProcessed.reduce(
        (groups: { [date: string]: Transaction[] }, tx) => {
          ...
          try {
            ...
            console.error(
              "Error formatting date for grouping:",
              groupDateStr,
              e
            );
            // Optionally group errors under a specific key
