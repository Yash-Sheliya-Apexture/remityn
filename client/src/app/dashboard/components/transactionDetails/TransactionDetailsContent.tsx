...
const TransactionDetailsContent: React.FC<TransactionDetailsContentProps> = ({
  ...
  const handleCopyToClipboard = useCallback(
    (text: string | undefined | null, fieldName: CopyableField) => {
      if (!text || copyStatus[fieldName]) {
        return;
      }
      navigator.clipboard
        ...
        .catch((err) => {
          console.error(`Failed to copy ${fieldName}: `, err);
          // It's good practice to inform the user, alert is basic but works.
          // Consider a more integrated notification system for better UX.
          alert(
            `Could not copy ${fieldName}. You may need to copy it manually.`
          );
          setCopyStatus((prevStatus) => ({
            ...prevStatus,
            [fieldName]: false,
  ...
  // Helper for number formatting to avoid repetition and ensure safety
          ...
          <div className="space-y-3 text-sm">
              ...
              <button
                ...
                onClick={() =>
                  handleCopyToClipboard(
                    paymentDetails.bankDetails?.payeeName,
                    "payeeName"
                  )
                }
                aria-label={`Copy Payee name`}
                className={`shrink-0 h-8 px-2.5 text-xs font-medium transition-colors duration-150 ease-in-out focus-visible:outline-none rounded-md flex items-center justify-center text-subheadingWhite bg-secondarybox hover:bg-secondaryboxhover
                                ...
                                `} // Removed redundant empty class string
                disabled={copyStatus.payeeName}
              >
                {copyStatus.payeeName ? (
                  <Check className={iconClasses} aria-hidden="true" />
                ...
                <span className="ml-1.5">
                  {copyStatus.payeeName ? "Copied!" : "Copy"}
                </span>
              </button>
            </div>
            {/* IBAN */}
            ...
            <div className="bg-primarybox p-4 rounded-lg flex justify-between items-center gap-2">
              ...
              <button
                ...
                onClick={() =>
                  handleCopyToClipboard(
                    paymentDetails.bankDetails?.bicSwift,
                    "bicSwift"
                  )
                }
                aria-label={`Copy BIC/SWIFT`}
              ...
              >
                ...
                <span className="ml-1.5">
                  {copyStatus.bicSwift ? "Copied!" : "Copy"}
                </span>
              </button>
            </div>
            {/* Reference Code */}
            {paymentDetails.referenceCode && (
              <div className="bg-primarybox p-4 rounded-lg flex justify-between items-center gap-2">
                ...
                >
                  ...
                  <span className="ml-1.5">
                    {copyStatus.referenceCode ? "Copied!" : "Copy"}
                  </span>
                </button>
              </div>
            )}
