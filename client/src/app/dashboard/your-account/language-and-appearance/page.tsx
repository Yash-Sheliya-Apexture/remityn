...
import React, { useState, useEffect, useCallback } from 'react';
import { IoMoonOutline, IoSunnyOutline, IoLanguageOutline, IoContrastOutline } from 'react-icons/io5';
import { GoChevronRight } from 'react-icons/go';
import Link from 'next/link';
import DashboardHeader from '@/app/components/layout/DashboardHeader';
...
// --- LanguageAndAppearancePage Component (Modified to match original) ---
const LanguageAndAppearancePage: React.FC = () => {
  // State for theme ('light', 'dark', or 'system')
  const [theme, setTheme] = useState<string>('system'); // Default to system
  const [language, setLanguage] = useState<string>('en-US');
  ...
  if (theme === 'dark' || (theme === 'system' && typeof window !== 'undefined' && window.matchMedia('(prefers-color-scheme: dark)').matches)) {
      ...
      currentThemeIcon = <IoContrastOutline className="size-5" />; // System icon
  }
