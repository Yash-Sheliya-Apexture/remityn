...
.refine(data => { // Check only if both dates are valid Date objects
    return !(data.idIssueDate instanceof Date && data.idExpiryDate instanceof Date) || data.idExpiryDate > data.idIssueDate;
    }, {
    message: "Expiry date must be after the issue date.",
    path: ["idExpiryDate"], // Apply error to expiry date field
});
...
export default function KycIdentityPage() {
    const {
        kycData, setKycData, nextStep, prevStep, updateCurrentUiStepId, goToStep,
        isInitialized: kycInitialized, backendStatus, isLoadingStatus: kycLoadingStatus, authLoading
    } = useKyc();
    ...
    }, [kycInitialized, authLoading, backendStatus, kycData.idType, kycData.idNumber, kycData.idIssueDate, kycData.idExpiryDate]);
    ...
    if (!['not_started', 'rejected', 'skipped'].includes(backendStatus as string)) {
        return ( <div className="flex justify-center items-center min-h-[400px]"><Loader2 className="h-8 w-8 animate-spin text-primary" /><span className="ml-2">Checking status...</span></div> );
    }
             ...
             <CardContent className="p-6 md:p-8">
                ...
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                        ...
                        {/* Conditionally Rendered Fields - Fade in/out */}
                        <div className={cn(
                            "space-y-6 pt-6 border-t border-border/30 dark:border-border/20 transition-opacity duration-300 ease-in-out",
                            selectedIdType ? "opacity-100" : "opacity-0 h-0 overflow-hidden pointer-events-none" // Hide completely when no type selected
                         )}>
                            {/* ID Number Field */}
                            <FormField control={form.control} name="idNumber" render={({ field }) => (
                                <FormItem>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-6">
                                <FormField control={form.control} name="idIssueDate" render={({ field }) => (
                                    <FormItem className="flex flex-col">
                                        <FormLabel>Date of Issue *</FormLabel>
                                        <Popover>
                                            <PopoverTrigger asChild><FormControl><Button type="button" variant={"outline"} className={cn("w-full justify-start text-left font-normal", !field.value && "text-muted-foreground")}> <CalendarIcon className="mr-2 h-4 w-4" /> {field.value && isDateValid(field.value) ? format(field.value, "PPP") : <span>Pick issue date</span>} </Button></FormControl></PopoverTrigger>
                                            <PopoverContent className="w-auto p-0" align="start"><Calendar mode="single" selected={field.value && isDateValid(field.value) ? field.value : undefined} onSelect={(date) => { field.onChange(date || undefined); if (form.getValues("idExpiryDate")) { form.trigger("idExpiryDate"); } }} disabled={(date) => date > startOfDay(new Date()) || date < new Date("1950-01-01")} initialFocus captionLayout="dropdown-buttons" fromYear={1950} toYear={new Date().getFullYear()} /></PopoverContent>
                                        </Popover>
                                        <FormMessage />
                                    </FormItem>
                                )} />
                                <FormField control={form.control} name="idExpiryDate" render={({ field }) => (
                                    <FormItem className="flex flex-col">
                                        ...
                                        <Popover>
                                            <PopoverTrigger asChild><FormControl><Button type="button" variant={"outline"} className={cn("w-full justify-start text-left font-normal", !field.value && "text-muted-foreground")}> <CalendarIcon className="mr-2 h-4 w-4" /> {field.value && isDateValid(field.value) ? format(field.value, "PPP") : <span>Pick expiry date</span>} </Button></FormControl></PopoverTrigger>
                                            <PopoverContent className="w-auto p-0" align="start"><Calendar mode="single" selected={field.value && isDateValid(field.value) ? field.value : undefined} onSelect={(date) => { field.onChange(date || undefined); form.trigger("idExpiryDate"); }} disabled={(date) => date < startOfDay(addDays(new Date(), 1))} initialFocus captionLayout="dropdown-buttons" fromYear={new Date().getFullYear()} toYear={new Date().getFullYear() + 30} /></PopoverContent>
                                        </Popover>
                                        <FormDescription>Your ID must not be expired.</FormDescription><FormMessage />
                                    </FormItem>
                                )} />
                        ...
                        <div className="flex justify-between items-center pt-8 border-t dark:border-border/50 mt-8">
                             <Button type="button" variant="outline" onClick={prevStep} disabled={isSubmitting}><ArrowLeft className="mr-2 h-4 w-4" /> Back</Button>
                             <Button type="submit" disabled={isSubmitting || !form.formState.isValid || !selectedIdType}>{isSubmitting ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <ArrowRight className="mr-2 h-4 w-4" />} Continue</Button>
                        </div>
                    </form>
                </Form>
            </CardContent>
