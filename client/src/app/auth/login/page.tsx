...
//   const searchParams = useSearchParams(); // useSearchParams from next/navigation
//   const [isSubmitting, setIsSubmitting] = useState(false);
//   const [sessionExpiredMessage, setSessionExpiredMessage] = useState("");
//   const [isGeneralErrorVisible, setIsGeneralErrorVisible] = useState(false);
//   const [loginSuccess, setLoginSuccess] = useState(false);
...
//     const googleErrParam = params.get("googleError");
//     const registerSuccessParam = params.get("registerSuccess");
//     const resetSuccessParam = params.get("resetSuccess");

//     let needsUrlClean = false;
//     let newSessionExpiredMsg = "";
//     let newGeneralErrorMsg = "";

//     // Determine messages based on URL params
//     if (googleErrParam) {
//       newGeneralErrorMsg = decodeURIComponent(googleErrParam);
//       needsUrlClean = true;
//     } else if (sessionExpiredParam === "true") {
//       newSessionExpiredMsg = "Your session has expired. Please log in again.";
//       needsUrlClean = true;
//     }

//     // These params don't set messages but require URL cleaning
//     if (registerSuccessParam === "true" || resetSuccessParam === "true") {
//       needsUrlClean = true;
//     }
    
//     // Update states
//     setSessionExpiredMessage(newSessionExpiredMsg);
//     setGeneralError(newGeneralErrorMsg);
//     // isGeneralErrorVisible will be updated by its own useEffect based on generalError

//     if (needsUrlClean) {
//       // Use router.replace to clean URL params, ensuring Next.js router state is updated
//       // This helps `searchParams` hook get the correct (empty) params on the next render cycle.
//       const currentPathname = window.location.pathname; // Or derive from router if needed, but pathname is fine
//       router.replace(currentPathname, { scroll: false });
//     }
//   // eslint-disable-next-line react-hooks/exhaustive-deps
//   }, [searchParams, router]); // Added router to dependency array as it's used

//   useEffect(() => {
//     if (!loading && user) {
//       // AuthContext should handle the redirect logic.
//     }
//   }, [user, loading, router]);
...
//     setPasswordError("");
//     setGeneralError("");
//     setSessionExpiredMessage(""); // Explicitly clear messages on new submission attempt
//     setLoginSuccess(false);
    
//     // isGeneralErrorVisible and isLoginSuccessVisible will be updated by their useEffects
...
//   const handleGoogleLogin = () => {
//     setGeneralError("");
//     setSessionExpiredMessage(""); // Explicitly clear messages
//     // isGeneralErrorVisible will be updated by its useEffect
//     window.location.href = `${apiConfig.baseUrl}/auth/google`;
//   };
...
//                     setEmail(e.target.value);
//                     if (emailError) setEmailError("");
//                     if (sessionExpiredMessage) setSessionExpiredMessage(""); // Clear session message on input interaction
//                     if (generalError) setGeneralError(""); // Clear general message on input interaction
//                   }}
//                   aria-invalid={!!emailError}
//                 />
//                 {emailError && (
...
export default function LoginPage() {
    ...
    const clearMessages = () => {
        ...
        setNotVerifiedError(false); setSuccessMessage("");
    };
    ...
    const handleResendVerification = () => {
        ...
        router.push(`/auth/register?email=${encodeURIComponent(email)}&verify=true`);
    };
    ...
    return (
        <div className="flex flex-col lg:flex-row min-h-screen lg:p-3">
             <div className="hidden lg:flex lg:w-[55%] xl:w-[60%] bg-[url(/assets/images/leftPartImage.png)] bg-cover bg-no-repeat bg-center p-10 xl:p-16 rounded-3xl flex-col justify-between relative">
                ...
                <div className="flex-grow flex flex-col justify-end items-start text-white pt-20">
                    <motion.h1 className="text-4xl xl:text-5xl font-bold mb-4" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }}>Get Started<br/>with Us</motion.h1>
                    <motion.p className="text-base xl:text-lg text-gray-200 mb-10 xl:mb-12 leading-relaxed" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.4 }}>Complete these easy steps<br/>to register your account.</motion.p>
                    <div className="flex xl:flex-row flex-col gap-4 w-full">
                        {[{ num: 1, title: "Sign up your account", active: false }, { num: 2, title: "Sign in your account", active: true }, { num: 3, title: "Set up your KYC", active: false }].map((s) => (
            ...
            <div className="w-full lg:w-[45%] xl:w-[40%] flex flex-col items-center justify-center p-6 sm:p-8 md:p-10 lg:p-12 xl:p-16 overflow-y-auto">
                ...
                <motion.div className="w-full max-w-sm md:max-w-md" key="login-form-container" variants={pageContainerVariants} initial="hidden" animate="visible" exit="exit">
                    <motion.div className="space-y-1 mb-6 lg:text-left text-center" variants={itemVariants}>
                      <h2 className="text-3xl lg:text-4xl text-mainheadingWhite font-semibold">Welcome back.</h2>
                      <p className="text-gray-400">New to Remityn? <Link href="/auth/register" className="text-primary hover:text-primaryhover font-medium underline underline-offset-2 transition-all duration-75 ease-linear">Sign up</Link></p>
                    </motion.div>
                    
                    <AnimatePresence>
                        {successMessage && (
                            <motion.div className="bg-primary-foreground p-4 rounded-xl flex items-center gap-3 mb-5" variants={errorVariants} initial="initial" animate="animate" exit="exit"><div className="flex bg-green-600/20 justify-center rounded-full items-center size-10 shrink-0"><FiCheckCircle className="text-green-500 size-5" /></div><div><p className="text-white">{successMessage}</p></div></motion.div>
                        )}
                        {generalError && (
                    ...
                    <form className="space-y-4" onSubmit={handleSubmit} noValidate>
                        <motion.div variants={itemVariants}><button type="button" onClick={handleGoogleLogin} className="flex hover:bg-primarybox transition-all duration-75 ease-linear border justify-center rounded-lg h-14 text-white w-full cursor-pointer font-medium gap-2.5 items-center px-4 py-3"><Image src="/assets/icon/google.svg" width={25} height={24} alt="Google" />Continue with Google</button></motion.div>
                        ...
                        <motion.div className="text-right pt-1" variants={itemVariants}>
                            <Link href="/auth/forgot-password" className="inline-block"><span className="text-subheadingWhite hover:text-primary font-medium underline underline-offset-2 transition-all duration-75 ease-linear">Forgot Password?</span></Link>
                        </motion.div>
                        
                        <motion.div variants={itemVariants} className="pt-2">
                            <button type="submit" disabled={isSubmitting || !!successMessage} className={`bg-primary hover:bg-primaryhover w-full text-mainheading font-semibold py-3 px-8 h-14 rounded-lg transition-all duration-75 ease-linear flex items-center justify-center ${isSubmitting || !!successMessage ? "opacity-70 cursor-not-allowed" : "cursor-pointer"}`}>{isSubmitting ? "Logging in..." : "Log in"}</button>
                        </motion.div>
                    </form>
                </motion.div>
            </div>
