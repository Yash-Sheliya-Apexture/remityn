...
// Helper to get Live Rate (implementation remains the same)
const getLiveRate = async (sendCode, receiveCode) => {
     try {
        if (sendCode === receiveCode) return 1.0;
        const latestRatesDocument = await exchangeRateService.getLatestExchangeRates();
        if (!latestRatesDocument?.rates?.rates) {
            console.warn(`Live rates data unavailable for ${sendCode}/${receiveCode}`);
            return null;
        ...
        if (rateBaseToSend !== null && !isNaN(rateBaseToSend) && rateBaseToReceive !== null && !isNaN(rateBaseToReceive) && rateBaseToSend !== 0) {
            const preciseRate = rateBaseToReceive / rateBaseToSend;
            return parseFloat(preciseRate.toFixed(6));
        }
        console.warn(`Could not calculate live rate between ${sendCode} and ${receiveCode}`);
        return null;
    } catch (error) {
...
// Calculate Send Summary (implementation remains the same)
const calculateSendSummary = async (userId, sourceAccountId, recipientId, amount, isSendingAmount) => {
    // ... (no changes needed)
    ...
    const sourceCurrencyDoc = await Currency.findById(sourceAccount.currency);
    const recipientCurrencyDoc = await Currency.findById(recipient.currency);
    if (!sourceCurrencyDoc || !recipientCurrencyDoc) throw new Error('Could not load currency details.');
    ...
    const liveExchangeRate = await getLiveRate(sendCurrencyCode, receiveCurrencyCode);
    if (liveExchangeRate === null || liveExchangeRate <= 0 || !isFinite(liveExchangeRate)) {
        throw new Error(`Live exchange rate is currently unavailable for ${sendCurrencyCode} to ${receiveCurrencyCode}.`);
    }
    console.log(`Service: calculateSendSummary - Fetched Live Rate: ${liveExchangeRate.toFixed(6)}`);
    ...
    console.log(`Service: calculateSendSummary - Adjusted Rate (Precise): ${adjustedExchangeRate}`);

    if (adjustedExchangeRate <= 0 || !isFinite(adjustedExchangeRate)) {
        throw new Error("Error calculating final exchange rate after adjustment.");
    }
    ...
    if (!currentSourceAccount) throw new Error('Source account re-verification failed.');

    if (currentSourceAccount.balance < sendAmountCalc) {
        console.warn(`Service: calculateSendSummary - Insufficient balance. Required: ${sendAmountCalc.toFixed(6)}, Available: ${currentSourceAccount.balance.toFixed(2)}`);
        throw new Error('Insufficient balance.');
    }
    ...
    const summary = {
        ...
        sendCurrencyCode,
        receiveCurrencyCode,
        exchangeRate: parseFloat(adjustedExchangeRate.toFixed(4)),
        liveExchangeRate: parseFloat(liveExchangeRate.toFixed(4)),
...
// Execute Transfer (implementation remains the same)
const executeTransfer = async (transferDetails) => {
    // ... (no changes needed)
    console.log('Service: executeTransfer - Start', transferDetails);
    const { userId, sourceAccountId, recipientId, sendAmount, receiveAmount, exchangeRate, reason, reference, sendCurrencyCode, receiveCurrencyCode } = transferDetails;

    if (!userId || !sourceAccountId || !recipientId || !sendCurrencyCode || !receiveCurrencyCode) throw new Error("Missing essential transfer information (IDs, currency codes).");
    if (sendAmount === undefined || sendAmount === null || isNaN(sendAmount) || sendAmount <= 0) throw new Error("Invalid or missing send amount.");
    if (receiveAmount === undefined || receiveAmount === null || isNaN(receiveAmount) || receiveAmount <= 0) throw new Error("Invalid or missing receive amount.");
    if (exchangeRate === undefined || exchangeRate === null || isNaN(exchangeRate) || exchangeRate <= 0) throw new Error("Invalid or missing exchange rate.");
    ...
    try {
        ...
        sourceAccount.balance -= sendAmount;
        await sourceAccount.save({ session });
        console.log(`Service: executeTransfer - Debited ${sendAmount} from account ${sourceAccountId}. Original: ${originalBalance}, New: ${sourceAccount.balance}`);
        ...
        await newTransfer.save({ session });
        console.log(`Service: executeTransfer - Created Transfer record ${newTransfer._id}`);

        await session.commitTransaction();
        console.log('Service: executeTransfer - Transaction committed.');

        populatedTransfer = await getTransferDetails(newTransfer._id, userId);
        ...
        if (error.message.includes('Insufficient balance') || error.message.includes('not found') || error.message.includes('mismatch') || error.message.includes('Invalid') || error.message.includes('Missing')) {
            throw error;
        }
...
const getTransferDetails = async (transferId, userId) => {
    ...
    if (!userId || !mongoose.Types.ObjectId.isValid(userId)) {
        console.error(`Service: getTransferDetails - Invalid or missing userId: ${userId}`);
        throw new Error('User authentication information is missing or invalid.');
    }

    const query = { _id: transferId, user: userId };
    const populateOptions = [
        { path: 'user', select: 'fullName email' },
        { path: 'sourceAccount', select: 'currency', populate: { path: 'currency', select: 'code flagImage currencyName _id' } },
        { path: 'recipient', select: 'accountHolderName nickname currency accountNumber bankName _id', populate: { path: 'currency', select: 'code flagImage currencyName _id' } },
        { path: 'sendCurrency', select: 'code flagImage currencyName _id' },
        { path: 'receiveCurrency', select: 'code flagImage currencyName _id' }
    ];

    try {
        const transfer = await Transfer.findOne(query).populate(populateOptions);

        if (!transfer) {
            // Log less severely for the expected "not found" case during fallback
            console.log(`Service: getTransferDetails - Transfer ${transferId} not found for user ${userId} (or access denied).`);
            throw new Error('Transfer not found or access denied.'); // Still throw the error for controller/frontend
        }
        console.log(`Service: getTransferDetails - Transfer ${transferId} found for user ${userId}.`);
        return transfer;
    } catch (error) {
         // --- Refined Logging ---
         if (error instanceof Error && error.message.includes('Transfer not found or access denied')) {
             // This was the expected error we threw above, no need to log as a "DB Error" again.
             // Just re-throw it for the controller.
             throw error;
         } else {
             // Log other potential DB errors more clearly
             console.error(`Service: getTransferDetails - Unexpected DB Error fetching transfer ${transferId} for user ${userId}:`, error.message);
             if (error instanceof Error && error.message.includes('Invalid')) {
                 throw error; // Re-throw format errors
             }
             // Throw a generic error for other unexpected DB issues
             throw new Error(`Failed to fetch transfer details due to database error.`);
         }
         // --- End Refined Logging ---
    }
};

// Get User Transfers (implementation remains the same)
const getUserTransfers = async (userId) => {
    // ... (no changes needed)
    console.log(`Service: getUserTransfers - Fetching transfers for user ${userId}`);
    ...
    try {
        const transfers = await Transfer.find({ user: userId })
            ...
            .populate('sendCurrency', 'code flagImage')
            .populate('receiveCurrency', 'code flagImage')
            .select('recipient sendCurrency receiveCurrency sendAmount receiveAmount status createdAt _id failureReason')
            .sort({ createdAt: -1 })
            .lean();
         ...
         console.log(`Service: getUserTransfers - Found ${transfersWithType.length} transfers.`);
...
// Cancel Transfer (implementation remains the same)
const cancelTransfer = async (transferId, userId) => {
    // ... (no changes needed)
    console.log(`Service: cancelTransfer - Attempting to cancel transfer ${transferId} for user ${userId}`);
    if (!mongoose.Types.ObjectId.isValid(transferId) || !mongoose.Types.ObjectId.isValid(userId)) {
        throw new Error('Invalid transfer or user ID format.');
    }
    ...
    try {
        ...
        if (transfer.status !== 'pending') {
            console.warn(`Service: cancelTransfer - Blocked cancellation for transfer ${transferId}. Current status: '${transfer.status}'. Required: 'pending'.`);
            throw new Error(`Cannot cancel transfer: Status must be pending (current: '${transfer.status}').`);
        }

        const needsRefund = true;

        if (needsRefund && transfer.sourceAccount && transfer.sendAmount > 0) {
            const sourceAccount = await Account.findById(transfer.sourceAccount).session(session);
            if (!sourceAccount) {
                console.error(`Service: cancelTransfer - Critical error: Source account ${transfer.sourceAccount} not found for pending transfer ${transferId}.`);
                throw new Error('Failed to cancel transfer: Source account data inconsistency.');
            }
            sourceAccount.balance += transfer.sendAmount;
            await sourceAccount.save({ session });
            console.log(`Service: cancelTransfer - Refunded ${transfer.sendAmount} to account ${transfer.sourceAccount}. New balance: ${sourceAccount.balance}`);
        } else if (needsRefund) {
            console.warn(`Service: cancelTransfer - Refund expected for pending transfer ${transferId}, but source account or send amount invalid. Skipping refund step.`);
        }
        ...
        populatedTransfer = await getTransferDetails(transfer._id, userId);

    } catch (error) {
        if (session.inTransaction()) {
            await session.abortTransaction();
            console.error(`Service: cancelTransfer - Error during cancellation transaction for ${transferId}, aborted:`, error);
        } else {
             console.error(`Service: cancelTransfer - Error occurred for ${transferId} outside active transaction:`, error);
        }

        if (error.message.startsWith('Cannot cancel transfer: Status must be pending') ||
            error.message.includes('not found') ||
            error.message.includes('access denied') ||
            error.message.includes('Invalid transfer') ||
