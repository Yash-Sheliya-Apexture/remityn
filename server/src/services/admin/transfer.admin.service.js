...
// Define allowed target statuses for admin updates
const ALLOWED_STATUS_UPDATES = ['processing', 'completed', 'failed', 'canceled'];
// Define states from which an update is forbidden
...
const getTransferByIdAdmin = async (transferId) => {
    ...
    try {
        // Consistent population strategy
         const populateOptions = [
            { path: 'user', select: 'fullName email' },
            { path: 'sourceAccount', select: 'currency', populate: { path: 'currency', select: 'code flagImage currencyName _id' } }, // Populate source account's currency fully
            { path: 'recipient', select: 'accountHolderName nickname currency accountNumber bankName _id', populate: { path: 'currency', select: 'code flagImage currencyName _id' } }, // Populate recipient and its currency fully
            { path: 'sendCurrency', select: 'code flagImage currencyName _id' }, // Populate send currency fully
            { path: 'receiveCurrency', select: 'code flagImage currencyName _id' } // Populate receive currency fully
        ];
        const transfer = await Transfer.findById(transferId).populate(populateOptions);

        if (!transfer) {
            throw new Error('Transfer not found.');
        }
        console.log(`Service: getTransferByIdAdmin - Transfer ${transferId} found.`);
        return transfer;
    } catch (error) {
         console.error(`Service: getTransferByIdAdmin - Error fetching transfer ${transferId}:`, error.message);
         if (error instanceof Error && (error.message.includes('Invalid transfer ID format') || error.message.includes('Transfer not found'))) {
             throw error;
         }
         throw new Error(`Failed to fetch transfer details for admin: ${error.message}`);
...
// --- Service to update transfer status by admin (WITH REFUND LOGIC) ---
const updateTransferStatusAdmin = async (transferId, newStatus, failureReason = null) => {
    console.log(`[Admin Transfer Service] Attempting update: ID=${transferId}, Status='${newStatus}', Reason='${failureReason}'`);

    if (!mongoose.Types.ObjectId.isValid(transferId)) {
        throw new Error('Invalid transfer ID format.');
    }
    if (!ALLOWED_STATUS_UPDATES.includes(newStatus)) {
        throw new Error(`Invalid status update value. Allowed: ${ALLOWED_STATUS_UPDATES.join(', ')}`);
    }
    ...
    try {
        ...
        console.log(`[Admin Transfer Service] Found Transfer ${transferId}. Current: '${originalStatus}', Target: '${newStatus}'.`);

        // --- Prevent updates FROM terminal states ---
        if (TERMINAL_STATUSES.includes(originalStatus)) {
            console.warn(`[Admin Transfer Service] Update blocked: Transfer ${transferId} is already in terminal state '${originalStatus}'.`);
            throw new Error(`Cannot update status for a transfer that is already ${originalStatus}.`);
        }

         // --- Prevent setting same status ---
         if (originalStatus === newStatus) {
              console.warn(`[Admin Transfer Service] Update skipped: Transfer ${transferId} is already in state '${newStatus}'.`);
              await session.abortTransaction();
              session.endSession();
              return await getTransferByIdAdmin(transferId); // Return existing populated data
         }
        ...
        if (needsRefund) {
            console.log(`[Admin Transfer Service] Refund condition met for transfer ${transferId}.`);
            if (transfer.sourceAccount && transfer.sendAmount > 0) {
                const sourceAccount = await Account.findById(transfer.sourceAccount).session(session);
                if (!sourceAccount) {
                    console.error(`[Admin Transfer Service] CRITICAL: Source account ${transfer.sourceAccount} not found for transfer ${transferId} during refund.`);
                    throw new Error('Failed to update status: Source account data missing for refund.');
                }
                // Add the sent amount back
                sourceAccount.balance += transfer.sendAmount;
                await sourceAccount.save({ session });
                refundProcessed = true;
                console.log(`[Admin Transfer Service] Refunded ${transfer.sendAmount} to account ${transfer.sourceAccount}. New balance: ${sourceAccount.balance}.`);
            } else {
                 console.warn(`[Admin Transfer Service] Refund needed for transfer ${transferId}, but source account (${transfer.sourceAccount}) or send amount (${transfer.sendAmount}) is invalid/zero. Skipping refund.`);
            }
        }

        // Update transfer status and potentially failure reason
        transfer.status = newStatus;
        // Mongoose timestamps handle updatedAt

        if ((newStatus === 'failed' || newStatus === 'canceled') && failureReason) {
            transfer.failureReason = failureReason;
            console.log(`[Admin Transfer Service] Set failureReason for ${transferId}: '${failureReason}'`);
        } else if (newStatus !== 'failed' && newStatus !== 'canceled') {
             // Clear reason if moving to processing/completed
             transfer.failureReason = undefined;
        }

        // Optional: Trigger external actions based on status change
        if (originalStatus === 'pending' && newStatus === 'processing') {
             console.log(`[Admin Transfer Service] Status changed ${originalStatus} -> ${newStatus} for ${transferId}. Consider triggering payout.`);
        } else if (originalStatus === 'processing' && newStatus === 'completed') {
             console.log(`[Admin Transfer Service] Status changed ${originalStatus} -> ${newStatus} for ${transferId}. Consider notifications.`);
        }

        await transfer.save({ session });
        await session.commitTransaction();
        console.log(`[Admin Transfer Service] Transfer ${transferId} updated to '${newStatus}'. Refund processed: ${refundProcessed}. Transaction committed.`);

        // Return the updated and populated transfer
        return await getTransferByIdAdmin(transferId); // Reuse population logic

    } catch (error) {
        await session.abortTransaction();
        console.error(`[Admin Transfer Service] Error updating transfer ${transferId}:`, error);
        // Re-throw specific known errors
        if (error.message.includes('Invalid status') ||
            error.message.includes('Transfer not found') ||
            error.message.includes('Cannot update status') || // From terminal state check
            error.message.includes('Source account data missing') ||
            error.message.includes('Invalid transfer ID format'))
        {
            throw error;
        }
        throw new Error(`Failed to update transfer status: ${error.message}`); // Generic fallback
    } finally {
         session.endSession();
    }
};
